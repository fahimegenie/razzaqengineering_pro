...
<section class="locations-section-pro" id="locationsSection">
    <div class="container">
        ...
        <div class="row g-5 align-items-center">
            ...
            <div class="col-lg-6" data-aos="fade-right">
                <div class="locations-content">
                    ...
                    <div class="cities-grid" id="citiesGrid">
                        ...
                        {{-- Main Cities Grid Items (Featured & Main Offices) --}}
                        @foreach($featuredCities as $city)
                            @php
                                $isMainOffice = in_array(ucwords(strtolower($city->name)), $mainOfficesList);
                            @endphp
                            <div class="city-card {{ $isMainOffice ? 'city-highlight' : '' }}" 
                                 data-city="{{ $city->name }}"
                                 data-lat="{{ $city->lat }}"
                                 data-lng="{{ $city->lng }}"
                                 data-main="{{ $isMainOffice ? 'true' : 'false' }}"
                                 onclick="focusCity('{{ $city->name }}', {{ $city->lat }}, {{ $city->lng }})">
                                <div class="city-icon">
                                    <i class="fas {{ $isMainOffice ? 'fa-building' : 'fa-city' }}"></i>
                                </div>
                                <div class="city-info">
                                    <span class="city-name">{{ $city->name }}</span>
                                    <span class="city-status" style="color: {{ $isMainOffice ? '#28a745' : '#0056b3' }};">
                                        {{ $isMainOffice ? 'Main Office' : 'Service Available' }}
                                    </span>
                                </div>
                        ...
                        {{-- Remaining Cities (Hidden by default) --}}
                    ...
                    <div class="view-all-wrapper">
                        ...
                        <span class="city-count" id="cityCount">
                            Showing {{ $featuredCities->count() }} of {{ $allCities->count() }} Cities
                        </span>
                    </div>
                    
                    {{-- CTA --}}
                    <div class="locations-cta">
                        <a href="{{ route('quote.index') }}" class="btn-locations-quote">
            ...
            {{-- Right: Map Container (Leaflet Dotted Map) --}}
                    ...
                    <div class="map-card">
                        ...
                        <div class="map-container" style="position: relative;">
                            ...
                            <div class="map-badge">
                                ...
                                <span>{{ $allCities->count() }}+ Cities Covered</span>
                            </div>
                        </div>
                        ...
                        <div class="map-stats">
                            ...
                            <div class="map-stat-item">
                                <span class="stat-number">{{ $allCities->count() }}+</span>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const totalCities = {{ $allCities->count() }};
        const featuredCount = {{ $featuredCities->count() }};
