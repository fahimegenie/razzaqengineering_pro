<div>
    <div class="app-content-header">
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-sm-6"><ol class="breadcrumb float-sm-end"><li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li><li class="breadcrumb-item active">Project Categories</li></ol></div>
            </div>
        </div>
    </div>
    ...
    <div class="container-fluid">
        <div class="row g-3 mb-3">
            <div class="col-6"><div class="card border-0 shadow-sm bg-primary text-white"><div class="card-body text-center py-3"><i class="bi bi-folder display-6"></i><h3 class="mb-0 mt-1">{{ $totalCategories }}</h3><small>Total Categories</small></div></div></div>
        ...
        <div class="card shadow-sm border-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    <thead class="table-light"><tr><th width="40"><input type="checkbox" class="form-check-input" @checked($selectAll) wire:click="toggleSelectAll"></th><th width="60">Img</th><th>Name</th><th>Slug</th><th>Projects</th><th width="90">Status</th><th width="80">Order</th><th width="140">Actions</th></tr></thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="badge bg-info">{{ $cat->projects_count }}</span></td>
                            ...
                            <td><div class="btn-group btn-group-sm"><button class="btn btn-info" wire:click="viewCategoryDetails({{ $cat->id }})"><i class="bi bi-eye"></i></button><a href="{{ route('admin.projects.categories.edit', $cat->id) }}" class="btn btn-primary"><i class="bi bi-pencil"></i></a><button class="btn btn-danger" wire:click="confirmDelete({{ $cat->id }})"><i class="bi bi-trash"></i></button></div></td>
                        </tr>
                        @empty
                        <tr><td colspan="8" class="text-center py-5"><i class="bi bi-folder2-open display-4 text-muted d-block"></i>No categories found<br><a href="{{ route('admin.projects.categories.create') }}" class="btn btn-primary mt-2"><i class="bi bi-plus-lg"></i> Add Category</a></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <!-- View Modal -->
    @if($showViewModal && $viewCategory)
    <div class="modal fade show d-block" style="background:rgba(0,0,0,0.5);z-index:1055;"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-header bg-primary text-white"><h5 class="modal-title"><i class="bi bi-eye"></i> Category Details</h5><button class="btn-close btn-close-white" wire:click="closeModals"></button></div><div class="modal-body text-center"><img src="{{ $viewCategory->image_url }}" class="rounded mb-3" style="max-height:150px;"><h5>{{ $viewCategory->pc_name }}</h5><p>{!! $viewCategory->pc_description !!}</p><span class="badge bg-info">{{ $viewCategory->projects_count }} Projects</span></div><div class="modal-footer"><button class="btn btn-secondary" wire:click="closeModals">Close</button><a href="{{ route('admin.projects.categories.edit', $viewCategory->id) }}" class="btn btn-primary"><i class="bi bi-pencil"></i> Edit</a></div></div></div></div><div class="modal-backdrop fade show"></div>
    @endif
