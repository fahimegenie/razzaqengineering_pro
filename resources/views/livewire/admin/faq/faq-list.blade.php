        ...
        <div class="card shadow-sm border-0 mb-3">
            <div class="card-body">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-6 col-md-2">
                        ...
                        <select class="form-select" wire:model.change="categoryFilter">
                            ...
                            <!-- Add wire:key here -->
                            <option value="{{ $cat }}" wire:key="category-opt-{{ $cat }}">{{ $cat }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-6 col-md-2 text-end">
                        <a href="{{ route('admin.faq.create') }}" class="btn btn-primary w-100">
                ...
                <div class="mt-3 p-2 bg-light rounded d-flex align-items-center gap-2 flex-wrap">
                    <span class="fw-semibold me-2">{{ count($selectedFaqs) }} selected</span>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover table-striped mb-0">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th wire:click="sortBy('id')" style="cursor: pointer;" width="70">
                                    ID @if($sortField === 'id')<i class="bi bi-arrow-{{ $sortDirection === 'asc' ? 'up' : 'down' }}"></i>@endif
                                </th>
                                <th wire:click="sortBy('faq_question')" style="cursor: pointer;">
                                    Question @if($sortField === 'faq_question')<i class="bi bi-arrow-{{ $sortDirection === 'asc' ? 'up' : 'down' }}"></i>@endif
                                </th>
                                <th>Category</th>
                                <th wire:click="sortBy('is_active')" style="cursor: pointer;" width="100">
                                    Status @if($sortField === 'is_active')<i class="bi bi-arrow-{{ $sortDirection === 'asc' ? 'up' : 'down' }}"></i>@endif
                                </th>
                                <th wire:click="sortBy('sort_order')" style="cursor: pointer;" width="100">
                                    Order @if($sortField === 'sort_order')<i class="bi bi-arrow-{{ $sortDirection === 'asc' ? 'up' : 'down' }}"></i>@endif
                                </th>
                        ...
                        <tbody>
                            @forelse($faqs as $faq)
                            <tr wire:key="faq-row-{{ $faq->id }}">
                                <td>
                                    <input type="checkbox" class="form-check-input"  @checked(in_array($faq->id, $selectedFaqs)) wire:click="toggleFaqSelection({{ $faq->id }})">
                                </td>
                                <td class="fw-semibold">#{{ $faq->id }}</td>
                                <td>
                                    <span class="text-truncate d-inline-block" style="max-width: 300px;" title="{{ $faq->faq_question }}">
                                        {{ Str::limit($faq->faq_question, 80) }}
                                    </span>
                                </td>
                                <td>
                                    @if($faq->faq_category)
                                    <span class="badge bg-info">{{ $faq->faq_category }}</span>
                                    @else
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        <button class="btn btn-info" wire:click="viewFaqDetails({{ $faq->id }})" title="View">
                                            <i class="bi bi-eye"></i>
                                        </button>
                                        <a href="{{ route('admin.faq.edit', $faq->id) }}" class="btn btn-primary" title="Edit">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <button class="btn btn-danger" wire:click="confirmDelete({{ $faq->id }})" title="Delete">
                            ...
                            <tr>
                                <td colspan="7" class="text-center py-5">
                                    <div class="text-muted">
                                        ...
                                        <a href="{{ route('admin.faq.create') }}" class="btn btn-sm btn-primary mt-2">
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable">
            <div class="modal-content shadow-lg border-0">
                ...
                <div class="modal-body">
                    <div class="mb-4">
                        ...
                        <div class="d-flex align-items-start gap-2 mt-1">
                            ...
                            <p class="fs-5 fw-semibold mb-0">{{ $viewFaq->faq_question }}</p>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            <div class="p-3 bg-light rounded-3 text-center">
                                ...
                                @if($viewFaq->faq_category)
                                    <span class="badge bg-info fs-6 mt-1">{{ $viewFaq->faq_category }}</span>
                                @else
                        ...
                        <div class="col-md-4">
                            <div class="p-3 bg-light rounded-3 text-center">
                                ...
                                <span class="badge bg-{{ $viewFaq->is_active ? 'success' : 'danger' }} fs-6 mt-1">
                                    {{ $viewFaq->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="p-3 bg-light rounded-3 text-center">
                                ...
                                <span class="badge bg-secondary fs-6 mt-1">{{ $viewFaq->sort_order }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row mt-3">
                        <div class="col-md-6">
                            <small class="text-muted">
                                ...
                                Created: {{ $viewFaq->created_at ? $viewFaq->created_at->format('M d, Y h:i A') : 'N/A' }}
                            </small>
                        </div>
                        <div class="col-md-6 text-md-end">
                            <small class="text-muted">
                                <i class="bi bi-calendar-check me-1"></i> 
                                Updated: {{ $viewFaq->updated_at ? $viewFaq->updated_at->format('M d, Y h:i A') : 'N/A' }}
                            </small>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-footer bg-light">
                    ...
                    <a href="{{ route('admin.faq.edit', $viewFaq->id) }}" class="btn btn-primary">
