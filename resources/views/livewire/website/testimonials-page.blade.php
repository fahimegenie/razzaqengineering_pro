<div class="testimonials-page-wrapper"
    ...
    <section class="testi-section">
        <div class="container">
            ...
            @if($isLoading)
                <div class="text-center py-5" wire:key="testi-loading">
                    <div class="spinner-border text-success" style="width:3rem;height:3rem;"></div>
                    <p class="text-muted mt-2">Loading testimonials...</p>
                </div>
            @elseif($errorMessage)
                <div class="alert alert-danger text-center rounded-3 border-0 shadow-sm" wire:key="testi-error">
                    <i class="fas fa-exclamation-triangle me-2"></i> {{ $errorMessage }}
                </div>
            @else
                <div wire:key="testi-main-content">
                        ...
                        <div class="row g-3 text-center">
                            <div class="col-md-4">
                                <div class="testi-stat-item">
                                    <span class="testi-stat-number">{{ $totalTestimonials }}+</span>
                                    <span class="testi-stat-label">Happy Clients</span>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="testi-stat-item">
                                    <span class="testi-stat-number">{{ $averageRating }}</span>
                            ...
                            <div class="col-md-4">
                                <div class="testi-stat-item">
                                    <span class="testi-stat-number">{{ count($cities) }}+</span>
                    ...
                    <div class="testi-filters" data-aos="fade-up" wire:ignore.self>
                        <div class="row g-3 align-items-end mb-5">
                            ...
                            <div class="col-md-1 d-flex align-items-end">
                                @if($search || $filterRating !== 'all' || $filterCity !== 'all')
                    ...
                    {{-- Testimonials Grid --}}
                    @php $displayed = $this->displayedTestimonials; @endphp
                    
                    @if($displayed->count() > 0)
                        <div class="testi-grid" wire:key="testi-grid-{{ md5($search.$filterRating.$filterCity) }}">
                            <div class="row g-4">
                                @foreach($displayed as $item)
                                    <div class="col-lg-4 col-md-6" 
                                         data-aos="fade-up" 
                                         data-aos-delay="{{ $loop->index % 3 * 100 }}"
                                         wire:key="testi-item-{{ $item->id }}">
                                        <div class="testi-card">
                                            
                                            {{-- Rating Stars --}}
                                            <div class="testi-rating mb-2">
                                                @for($i = 1; $i <= 5; $i++)
                                                    <i class="fas fa-star{{ $i <= $item->t_rating ? ' text-warning' : ' text-muted' }}"></i>
                                                @endfor
                                            </div>
                                            ...
                                            <div class="testi-author">
                                                ...
                                                <div class="testi-author-info">
                                                    <h5 class="testi-author-name">{{ $item->t_name }}</h5>
                                                    @if($item->t_designation)
                                                        <span class="testi-author-role">{{ $item->t_designation }}</span>
                                                    @endif
                                                    @if($item->t_company)
                                                        <span class="testi-author-company">@ {{ $item->t_company }}</span>
                                                    @endif
                                                </div>
                                            </div>
                                            ...
                                            <div class="testi-meta mt-2">
                                                @if($item->t_location)
                                                    <span class="testi-location">
                                                        <i class="fas fa-map-marker-alt text-danger me-1"></i> {{ $item->t_location }}
                                                    </span>
                                                @endif
                                                @if($item->service)
                                                    <span class="testi-service">
                                                        <i class="fas fa-tools text-primary me-1"></i> {{ $item->service->os_name }}
                                                    </span>
                                                @endif
                                            </div>
                        ...
                        {{-- Load More --}}
                        @if($this->filteredTestimonials->count() > $loadedCount)
                    ...
                    @else
                        <div class="text-center py-5" data-aos="fade-up" wire:key="testi-empty">
