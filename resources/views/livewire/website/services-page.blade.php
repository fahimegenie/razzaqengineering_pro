<div class="services-page" 
    ...
    <div class="svc-mobile-cta d-lg-none">
        ...
        <a href="{{ route('quote.index') }}" class="svc-mobile-btn svc-mobile-quote">
                ...
                <div class="col-lg-8" data-aos="fade-up">
                    ...
                    <div class="svc-hero-stats">
                        <div class="svc-stat-item">
                            <span class="svc-stat-number">{{ $totalCount }}+</span>
                            <span class="svc-stat-label">Services</span>
                        </div>
                        <div class="svc-stat-item">
                            <span class="svc-stat-number">{{ $cities->count() }}+</span>
    ...
    <section class="svc-main-section">
        <div class="container">
            ...
            @else
                            ...
                            <div class="svc-input-wrapper svc-dropdown-wrapper">
                                ...
                                <div class="svc-dropdown-menu" x-show="open" x-transition>
                                    ...
                                    <div class="svc-dropdown-items">
                                        <button type="button" 
                                                class="svc-dropdown-item {{ $selectedCity === 'all' ? 'active' : '' }}"
                                        ...
                                        @foreach($filteredCities as $city)
                                            <button type="button" 
                                                    class="svc-dropdown-item {{ $selectedCity == $city->id ? 'active' : '' }}"
                    ...
                    <div class="svc-featured-section" data-aos="fade-up">
                        ...
                        <div class="row g-4">
                            @foreach($featuredServices as $fs)
                                <div class="col-lg-4 col-md-6" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}" wire:key="fs-{{ $fs->id }}">
                                    <div class="svc-card featured">
                                        <div class="svc-card-image">
                                            <img src="{{ $fs->image_url }}" alt="{{ $fs->os_name }}" loading="lazy">
                                            <div class="svc-card-badges">
                                                <span class="svc-badge-featured"><i class="fas fa-star"></i> Featured</span>
                                            </div>
                                            <div class="svc-card-overlay">
                                                <a href="{{ url('services/'.$fs->os_slug) }}" class="svc-overlay-btn">
                                        ...
                                        <div class="svc-card-body">
                                            @if($fs->os_icon)
                                                <div class="svc-card-icon">
                                                    <i class="{{ $fs->os_icon }}"></i>
                                                </div>
                                            @endif
                                            <h3>
                                                <a href="{{ url('services/'.$fs->os_slug) }}">{{ $fs->os_name }}</a>
                                            </h3>
                                            <p>{{ Str::limit($fs->os_short_description ?? $fs->os_description, 90) }}</p>
                                            <div class="svc-card-footer">
                                                <a href="{{ url('services/'.$fs->os_slug) }}" class="svc-card-link">
                                                    Learn More <i class="fas fa-arrow-right ms-1"></i>
                                                </a>
                                                <a href="{{ route('quote.index') }}" class="svc-card-quote">
                ...
                {{-- All Services Grid --}}
                @if($totalCount > 0)
                    <div class="svc-all-services" wire:key="services-{{ $selectedCity }}-{{ md5($search) }}">
                        <div class="row g-4">
                            @foreach($services as $service)
                                <div class="col-lg-4 col-md-6" 
                                    ...
                                    <div class="svc-card">
                                        <div class="svc-card-image">
                                            <img src="{{ $service->image_url }}" alt="{{ $service->os_name }}" loading="lazy">
                                            <div class="svc-card-overlay">
                                                <a href="{{ url('services/'.Str::slug($service->os_name)) }}" class="svc-overlay-btn">
                                                    <i class="fas fa-eye me-1"></i> View Details
                                                </a>
                                            </div>
                                        </div>
                                        <div class="svc-card-body">
                                            ...
                                            @endif
                                            <h3>
                                                <a href="{{ url('services/'.Str::slug($service->os_name)) }}">{{ $service->os_name }}</a>
                                            </h3>
                                            <p>{{ Str::limit($service->os_short_description ?? $service->os_description, 90) }}</p>
                                            ...
                                            @if($service->cityServices->count() > 0)
                                                <div class="svc-card-cities">
                                                    ...
                                                    <div class="svc-cities-tags">
                                                        @foreach($service->cityServices->take(3) as $cs)
                                                            <a href="{{ url($cs->city->slug . '/' . Str::slug($service->os_name)) }}" 
                                                               class="svc-city-tag">{{ $cs->city->name }}</a>
                                                        @endforeach
                                                        @if($service->cityServices->count() > 3)
                                                            <span class="svc-city-more">+{{ $service->cityServices->count() - 3 }}</span>
                                                        @endif
                                                    </div>
                                                </div>
                                            @endif
                                            ...
                                            <div class="svc-card-footer">
                                                <a href="{{ url('services/'.Str::slug($service->os_name)) }}" class="svc-card-link">
                                                    Learn More <i class="fas fa-arrow-right ms-1"></i>
                                                </a>
                                                <a href="{{ route('quote.index') }}" class="svc-card-quote">
                                                    <i class="fas fa-paper-plane"></i>
                        ...
                        {{-- Load More --}}
                        <div class="svc-load-more" wire:key="load-more-{{ $loadedCount }}">
                            @if($hasMore)
                                ...
                                <p class="svc-load-info">Showing {{ count($services) }} of {{ $totalCount }} services</p>
                            @else
                                <div class="svc-all-loaded">
                                    <i class="fas fa-check-circle"></i>
                                    <span>All {{ $totalCount }} services loaded</span>
                                </div>
                            @endif
                        </div>
                    </div>
    ...
    {{-- ============================================
         CITY SERVICE LINKS (SEO Section)
         ============================================ --}}
    @if($cityServiceLinks && count($cityServiceLinks) > 0)
                                    ...
                                    @foreach($item['services'] as $svc)
                                        <li>
                                            <a href="{{ url($item['city']->slug . '/' . Str::slug($svc->os_name)) }}">
                                                <i class="fas fa-angle-right me-1"></i> {{ Str::limit($svc->os_name, 25) }}
                                            </a>
                                        </li>
                                    @endforeach
                                </ul>
                                <a href="{{ url($item['city']->slug) }}" class="svc-city-view-all">
    ...
    <section class="svc-final-cta">
        <div class="container text-center">
            ...
            <div class="svc-final-cta-buttons">
                <a href="{{ route('quote.index') }}" class="svc-btn svc-btn-lg svc-btn-accent">
...
.svc-hero-bg {
    ...
    background: url('{{ asset("images/services-hero-bg.jpg") }}') center/cover no-repeat;
