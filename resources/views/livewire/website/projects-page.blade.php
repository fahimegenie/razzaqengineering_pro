                ...
                <div class="col-lg-8" data-aos="fade-up">
                    ...
                    <div class="pp-hero-stats">
                        <div class="pp-stat-item">
                            <span class="pp-stat-number">{{ $totalCount }}+</span>
                ...
                <div class="col-lg-4 d-none d-lg-block text-center" data-aos="fade-left">
                    <div class="pp-hero-image-wrapper">
                        <div class="pp-hero-image-frame">
                            <img src="{{ $featuredProjects->first() ? asset($featuredProjects->first()->p_image) : asset('images/project-hero.jpg') }}" 
        ...
        <div class="container">
            <div class="pp-filters-card" data-aos="fade-up">
                        ...
                        <div class="pp-input-wrapper pp-dropdown-wrapper">
                            ...
                            <div class="pp-dropdown-menu" x-show="open" x-transition>
                                ...
                                <div class="pp-dropdown-items">
                                    <button type="button" 
                                            class="pp-dropdown-item {{ $selectedCategory === 'all' ? 'active' : '' }}"
                                    ...
                                    @foreach($filteredCategories as $cat)
                                        <button type="button" 
                                                class="pp-dropdown-item {{ $selectedCategory == $cat->pc_id ? 'active' : '' }}"
                    ...
                    <div class="col-lg-3" x-data="{ open: false }" @click.outside="open = false">
                        ...
                        <div class="pp-input-wrapper pp-dropdown-wrapper">
                            ...
                            <div class="pp-dropdown-menu" x-show="open" x-transition>
                                ...
                                <div class="pp-dropdown-items">
                                    <button type="button" 
                                            class="pp-dropdown-item {{ $selectedCity === 'all' ? 'active' : '' }}"
                                    ...
                                    @foreach($filteredCities as $city)
                                        <button type="button" 
                                                class="pp-dropdown-item {{ $selectedCity == $city->id ? 'active' : '' }}"
                ...
                <div class="pp-status-pills">
                    <button class="pp-status-pill {{ $selectedStatus === 'all' ? 'active' : '' }}" 
                            wire:click="filterByStatus('all')">All</button>
                    <button class="pp-status-pill {{ $selectedStatus === 'completed' ? 'active' : '' }}" 
                            wire:click="filterByStatus('completed')">Completed</button>
                    <button class="pp-status-pill {{ $selectedStatus === 'ongoing' ? 'active' : '' }}" 
                            wire:click="filterByStatus('ongoing')">Ongoing</button>
                    <button class="pp-status-pill {{ $selectedStatus === 'planning' ? 'active' : '' }}" 
    ...
    <section class="pp-content-section">
        <div class="container">
            ...
            @else
                    ...
                    <div class="pp-featured-section" data-aos="fade-up">
                        ...
                        <div class="row g-4">
                            @foreach($featuredProjects as $fp)
                                <div class="col-lg-4 col-md-6" wire:key="fp-{{ $fp->id }}">
                                    <div class="pp-project-card featured" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                                        <div class="pp-card-image">
                                            <img src="{{ asset($fp->p_image) }}" 
                                                 alt="{{ $fp->p_title }}" 
                                                 loading="lazy">
                                            <div class="pp-card-badges">
                                                <span class="pp-badge-featured"><i class="fas fa-star"></i> Featured</span>
                                                @if($fp->p_status)
                                                    <span class="pp-badge-status status-{{ $fp->p_status }}">{{ ucfirst($fp->p_status) }}</span>
                                                @endif
                                            </div>
                                            <div class="pp-card-overlay">
                                                <a href="{{ route('project.detail', ['slug' => $fp->p_slug]) }}" class="pp-overlay-btn">
                                        ...
                                        <div class="pp-card-body">
                                            @if($fp->category)
                                                <span class="pp-card-category">{{ $fp->category->pc_name }}</span>
                                            @endif
                                            <h3>
                                                <a href="{{ route('project.detail', ['slug' => $fp->p_slug]) }}">{{ $fp->p_title }}</a>
                                            </h3>
                                            <p>{{ Str::limit($fp->p_short_description ?? $fp->p_description, 90) }}</p>
                                            <div class="pp-card-meta">
                                                @if($fp->p_location)
                                                    <span><i class="fas fa-map-marker-alt"></i> {{ $fp->p_location }}</span>
                                                @endif
                                                <span><i class="far fa-calendar-alt"></i> {{ $fp->p_start_date ? $fp->p_start_date->format('M Y') : 'N/A' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                {{-- All Projects Grid --}}
                @if($totalCount > 0)
                    <div class="pp-all-projects" wire:key="projects-{{ $selectedCategory }}-{{ $selectedCity }}-{{ $selectedStatus }}-{{ md5($search) }}">
                        <div class="row g-4">
                                ...
                                <div class="col-lg-4 col-md-6" 
                                    ...
                                    <div class="pp-project-card">
                                        <div class="pp-card-image">
                                            ...
                                            @if($project->p_status)
                                                <div class="pp-card-badges">
                                                    <span class="pp-badge-status status-{{ $project->p_status }}">{{ ucfirst($project->p_status) }}</span>
                                                </div>
                                            @endif
                                            <div class="pp-card-overlay">
                                                <a @if(!empty($project->p_slug)) href="{{ route('project.detail', ['slug' => $project->p_slug]) }}" @else href="#" @endif  class="pp-overlay-btn">
                                                    <i class="fas fa-eye me-1"></i> View Details
                                                </a>
                                            </div>
                                        </div>
                                        <div class="pp-card-body">
                                            @if($project->category)
                                                <span class="pp-card-category">{{ $project->category->pc_name }}</span>
                                            @endif
                                            <h3>
                                                <a @if(!empty($project->p_slug)) href="{{ route('project.detail', ['slug' => $project->p_slug]) }}" @else href="#" @endif >{{ $project->p_title }}</a>
                                            </h3>
                                            <p>{{ Str::limit($project->p_short_description ?? $project->p_description, 90) }}</p>
                                            <div class="pp-card-meta">
                                                @if($project->p_location)
                                                    <span><i class="fas fa-map-marker-alt"></i> {{ $project->p_location }}</span>
                                                @endif
                                                @if($project->p_client)
                                                    <span><i class="fas fa-building"></i> {{ $project->p_client }}</span>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                        ...
                        {{-- Load More --}}
                        <div class="pp-load-more" wire:key="load-more-{{ $loadedCount }}">
                            @if($hasMore)
                                ...
                                <p class="pp-load-info">Showing {{ count($projects) }} of {{ $totalCount }} projects</p>
                            @else
                                <div class="pp-all-loaded">
                                    <i class="fas fa-check-circle"></i>
                                    <span>All {{ $totalCount }} projects loaded</span>
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            <div class="pp-cta-card" data-aos="zoom-in">
                <div class="row align-items-center">
                    ...
                    <div class="col-lg-4 text-lg-end mt-3 mt-lg-0">
                        <a href="{{ route('quote.index') }}" class="pp-cta-btn">
...
.pp-hero-bg {
    ...
    background: url('{{ asset("images/construction-hero.jpg") }}') center/cover no-repeat;
