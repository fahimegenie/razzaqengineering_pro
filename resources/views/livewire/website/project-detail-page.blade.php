<div class="pdp-page" 
    ...
    <section class="pdp-hero">
        <div class="pdp-hero-bg" style="background-image: url('{{ $project->image_url ?? asset('images/hero-construction.jpg') }}');"></div>
            ...
            <div class="row align-items-center min-vh-30">
                <div class="col-lg-8" data-aos="fade-up">
                    ...
                    <div class="pdp-hero-badge">
                        ...
                        {{ $project->category->pc_name ?? 'Project' }}
                        @if($project->p_status)
                            <span class="pdp-hero-status status-{{ $project->p_status }}">{{ ucfirst($project->p_status) }}</span>
                        @endif
                    </div>
                    
                    <h1 class="pdp-hero-title">{{ $project->p_title ?? 'Project Details' }}</h1>
                    
                    <p class="pdp-hero-subtitle">{{ $project->p_short_description ?? 'Professional engineering project executed with precision and quality workmanship.' }}</p>
                    ...
                    <div class="pdp-hero-meta">
                        ...
                        @if($project->p_start_date)
                            <span><i class="far fa-calendar-alt"></i> {{ $project->p_start_date->format('M Y') }}</span>
                        @endif
                    </div>
                ...
                <div class="col-lg-4 d-none d-lg-block text-center" data-aos="fade-left">
                    <div class="pdp-hero-image-wrapper">
                        <img src="{{ $project->image_url ?? asset('images/project-default.jpg') }}" 
                             alt="{{ $project->p_title ?? 'Project' }}" 
                        ...
                        <div class="pdp-content-card" data-aos="fade-up">
                            ...
                            <div class="pdp-content-body">
                                ...
                                <div class="pdp-description">
                                    {!! Str::words(strip_tags($project->p_description ?? ''), 80, '...') !!}
                                </div>
                                
                                @if($project->p_description && Str::wordCount(strip_tags($project->p_description)) > 80)
                                ...
                                <div class="pdp-info-grid">
                                    @if($project->category)
                                        <div class="pdp-info-item-card">
                                            ...
                                            <span>Category</span>
                                            <strong>{{ $project->category->pc_name }}</strong>
                                        </div>
                                    @endif
                                    @if($project->p_status)
                                        <div class="pdp-info-item-card">
                                            ...
                                            <span>Status</span>
                                            <strong class="status-{{ $project->p_status }}">{{ ucfirst($project->p_status) }}</strong>
                                        </div>
                                    @endif
                                    @if($project->p_location)
                                        <div class="pdp-info-item-card">
                                            ...
                                            <span>Location</span>
                                            <strong>{{ $project->p_location }}</strong>
                                        </div>
                                    @endif
                                    @if($project->p_client)
                                        <div class="pdp-info-item-card">
                                            ...
                                            <span>Client</span>
                                            <strong>{{ $project->p_client }}</strong>
                                        </div>
                                    @endif
                                    @if($project->p_start_date)
                                        <div class="pdp-info-item-card">
                                            ...
                                            <strong>{{ $project->p_start_date->format('M d, Y') }}</strong>
                                        </div>
                                    @endif
                                    @if($project->p_end_date)
                                        <div class="pdp-info-item-card">
                                            ...
                                            <strong>{{ $project->p_end_date->format('M d, Y') }}</strong>
                                        </div>
                                    @endif
                                </div>
                            </div>
                                ...
                                <div class="pdp-sidebar-card">
                                    ...
                                    <div class="pdp-related-list">
                                        @foreach($relatedProjects as $rp)
                                            <a href="{{ route('project.detail', ['slug' => $rp->p_slug ?? $rp->id]) }}" class="pdp-related-item">
                                                <img src="{{ $rp->image_url }}" alt="{{ $rp->p_title }}" loading="lazy">
                                                <div>
                                                    <h6>{{ Str::limit($rp->p_title, 35) }}</h6>
                                                    <small><i class="fas fa-map-marker-alt me-1"></i>{{ $rp->p_location ?? 'N/A' }}</small>
                                                </div>
