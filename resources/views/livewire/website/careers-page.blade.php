<div class="car-page" x-data="{ search: @entangle('search'), activeDept: @entangle('selectedDepartment'), activeLoc: @entangle('selectedLocation'), activeType: @entangle('selectedType') }">
    ...
    <div class="car-mobile-cta d-lg-none">
        ...
        <a href="{{ route('quote.index') }}" class="car-mobile-btn car-mobile-quote">
            ...
            <div class="row align-items-center min-vh-30">
                <div class="col-lg-8" data-aos="fade-up">
                    ...
                    <div class="car-hero-stats">
                        <div class="car-stat-item"><span class="car-stat-number">{{ $totalCount }}+</span><span class="car-stat-label">Open Positions</span></div>
                        <div class="car-stat-item"><span class="car-stat-number">{{ count($departments) }}</span><span class="car-stat-label">Departments</span></div>
                        <div class="car-stat-item"><span class="car-stat-number">{{ count($locations) }}</span><span class="car-stat-label">Locations</span></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="car-main-section">
        <div class="container">
            ...
            @elseif($errorMessage)
                <div class="car-error-card"><i class="fas fa-exclamation-triangle" style="font-size:3rem;color:#dc3545;"></i><h4 class="fw-bold mt-3">Oops!</h4><p class="text-muted">{{ $errorMessage }}</p></div>
            @else
                {{-- Section Header --}}
                ...
                <div class="car-jobs-list" data-aos="fade-up">
                    @if($filteredJobs->count() > 0)
                        @foreach($filteredJobs as $job)
                            <div class="car-job-card" wire:key="job-{{ $job->id }}">
                                <div class="car-job-body">
                                    <div class="car-job-main">
                                        <h3>
                                            <a href="{{ route('careers.detail', ['slug' => $job->slug]) }}">{{ $job->title }}</a>
                                        </h3>
                                        <div class="car-job-meta">
                                            @if($job->department)<span class="car-job-tag"><i class="fas fa-building"></i> {{ $job->department }}</span>@endif
                                            @if($job->location)<span class="car-job-tag"><i class="fas fa-map-marker-alt"></i> {{ $job->location }}</span>@endif
                                            @if($job->job_type)<span class="car-job-tag"><i class="fas fa-clock"></i> {{ $job->job_type }}</span>@endif
                                        </div>
                                        <p>{{ Str::limit(strip_tags($job->description), 150) }}</p>
                                    </div>
                                    <div class="car-job-actions">
                                        <a href="{{ route('careers.detail', ['slug' => $job->slug]) }}" class="car-btn car-btn-outline">View Details</a>
                                        <a href="{{ route('careers.apply', ['slug' => $job->slug]) }}" class="car-btn car-btn-accent"><i class="fas fa-paper-plane me-1"></i> Apply Now</a>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                ...
                @foreach($benefits as $index => $benefit)
                    <div class="col-lg-4 col-md-6" data-aos="fade-up" data-aos-delay="{{ $index * 80 }}">
                        <div class="car-benefit-card">
                            <div class="car-benefit-icon"><i class="fas {{ $benefit['icon'] }}"></i></div>
                            <h4>{{ $benefit['title'] }}</h4>
                            <p>{{ $benefit['desc'] }}</p>
                        </div>
                    </div>
    ...
    <section class="car-final-cta">
        <div class="container text-center">
            ...
            <div class="car-final-cta-buttons">
                <a href="{{ route('home.contact') }}" class="car-btn car-btn-lg car-btn-accent"><i class="fas fa-envelope me-2"></i> Contact Us</a>
...
.car-hero-bg { position: absolute; inset: 0; background: url('{{ asset("images/careers-hero-bg.jpg") }}') center/cover no-repeat; filter: brightness(0.3); }
...
<script>document.addEventListener('livewire:navigated', () => { if (typeof AOS !== 'undefined') AOS.refresh(); });</script>