<div class="blog-page-wrapper"
    ...
    <section class="blog-section">
        <div class="container">
            ...
            @if($isLoading)
                <div class="text-center py-5" wire:key="blog-loading">
                    <div class="spinner-border text-success" style="width:3rem;height:3rem;"></div>
                </div>
            @elseif($errorMessage)
                <div class="alert alert-danger text-center rounded-3 border-0 shadow-sm" wire:key="blog-error">
                    <i class="fas fa-exclamation-triangle me-2"></i> {{ $errorMessage }}
                </div>
            @else
                <div wire:key="blog-main-content">
                    ...
                    <div class="row g-5">
                        ...
                        <div class="col-lg-8">
                            ...
                            {{-- Posts --}}
                            @if($posts->count() > 0)
                                <div class="blog-posts" wire:key="blog-posts-{{ md5($search.$selectedCategory.$selectedTag) }}">
                                    @foreach($posts as $post)
                                        <div class="blog-post-card mb-4" data-aos="fade-up" wire:key="post-{{ $post->id }}">
                                            <div class="row g-4">
                                                <div class="col-md-5" wire:ignore>
                                                    <a href="{{ route('blog.detail', $post->bp_slug) }}" class="blog-post-img-link">
                                                        <img src="{{ $post->image_url }}" alt="{{ $post->bp_title }}" class="blog-post-img rounded-3" loading="lazy">
                                                    </a>
                                                </div>
                                                <div class="col-md-7">
                                                    <div class="blog-post-meta mb-2">
                                                        @if($post->category)
                                                            <a href="?category={{ $post->category->bc_slug }}" class="blog-cat-tag">{{ $post->category->bc_name }}</a>
                                                        @endif
                                                        <span class="blog-date"><i class="far fa-calendar-alt me-1"></i> {{ $post->published_at?->format('M d, Y') ?? $post->created_at->format('M d, Y') }}</span>
                                                        <span class="blog-read-time"><i class="far fa-clock me-1"></i> {{ $post->reading_time ?? 5 }} min read</span>
                                                    </div>
                                                    <h3 class="blog-post-title">
                                                        <a href="{{ route('blog.detail', $post->bp_slug) }}">{{ $post->bp_title }}</a>
                                                    </h3>
                                                    <p class="blog-post-excerpt">{{ Str::limit($post->bp_excerpt ?? strip_tags($post->bp_content), 180) }}</p>
                                                    <a href="{{ route('blog.detail', $post->bp_slug) }}" class="blog-read-more">
                        ...
                        <div class="col-lg-4" data-aos="fade-left">
                            ...
                            <div class="blog-sidebar-card mb-4">
                                ...
                                <ul class="list-unstyled mb-0">
                                    <li class="mb-2">
                                        <a href="#" wire:click.prevent="filterByCategory('all')" class="blog-sidebar-link {{ $selectedCategory === 'all' ? 'active' : '' }}">
                                            All Categories
                                        </a>
                                    </li>
                                    @foreach($categories as $cat)
                                        <li class="mb-2">
                                            <a href="#" wire:click.prevent="filterByCategory('{{ $cat->id }}')" class="blog-sidebar-link {{ $selectedCategory == $cat->id ? 'active' : '' }}">
                                                {{ $cat->bc_name }} <span class="text-muted">({{ $cat->posts_count }})</span>
                                            </a>
                                        </li>
                                    @endforeach
                                </ul>
                            ...
                            @if($recentPosts->count() > 0)
                                <div class="blog-sidebar-card mb-4">
                                    ...
                                    @foreach($recentPosts as $rp)
                                        <a href="{{ route('blog.detail', $rp->bp_slug) }}" class="blog-recent-item">
                            ...
                            @if($tags->count() > 0)
                                <div class="blog-sidebar-card">
                                    ...
                                    <div class="d-flex flex-wrap gap-2">
                                        @foreach($tags as $tag)
                                            <a href="#" wire:click.prevent="filterByTag('{{ $tag->id }}')" 
                                               class="blog-tag-btn {{ $selectedTag == $tag->id ? 'active' : '' }}">
                                                {{ $tag->bt_name }}
                                            </a>
                                        @endforeach
                                    </div>
