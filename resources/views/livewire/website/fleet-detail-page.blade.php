<div class="fdp-page" 
    ...
    <div class="fdp-mobile-cta d-lg-none">
        ...
        <a href="{{ route('quote.index') }}" class="fdp-mobile-btn fdp-mobile-quote">
    ...
    <section class="fdp-hero">
        <div class="fdp-hero-bg" style="background-image: url('{{ $fleetItem->image_url ?? asset('images/fleet-hero-bg.jpg') }}');"></div>
            ...
            <div class="row align-items-center min-vh-30">
                <div class="col-lg-8" data-aos="fade-up">
                    <nav aria-label="breadcrumb">
                        <ol class="fdp-breadcrumb">
                            ...
                            <li><a href="{{ route('public.fleet') }}">Our Fleet</a></li>
                            @if($fleetItem && $fleetItem->category)
                                <li><a href="{{ url('our-fleet?category='.$fleetItem->category->slug) }}">{{ $fleetItem->category->name }}</a></li>
                            @endif
                            <li class="active">{{ $fleetItem->title ?? 'Fleet Details' }}</li>
                        </ol>
                    </nav>
                    
                    <div class="fdp-hero-badge">
                        <i class="fas fa-truck-monster"></i> 
                        {{ $fleetItem->category->name ?? 'Heavy Machinery' }}
                        @if(!empty($fleetItem->manufacturer))
                            <span class="fdp-hero-badge-sub">{{ $fleetItem->manufacturer }}</span>
                        @endif
                    </div>
                    
                    <h1 class="fdp-hero-title">{{ $fleetItem->title ?? 'Fleet Details' }}</h1>
                    
                    <p class="fdp-hero-subtitle">
                        {{ $fleetItem->description ? Str::limit($fleetItem->description, 120) : 'Professional heavy machinery available for your industrial and construction projects.' }}
                    </p>
                    
                    <div class="fdp-hero-meta">
                        @if($fleetItem->manufacturer)
                            <span><i class="fas fa-industry"></i> {{ $fleetItem->manufacturer }}</span>
                        @endif
                        @if($fleetItem->model_number)
                            <span><i class="fas fa-tag"></i> Model: {{ $fleetItem->model_number }}</span>
                        @endif
                        @if($fleetItem->category)
                            <span><i class="fas fa-layer-group"></i> {{ $fleetItem->category->name }}</span>
                        @endif
                    </div>
                    
                    <div class="fdp-hero-cta">
                        <a href="{{ route('quote.index') }}" class="fdp-btn fdp-btn-lg fdp-btn-accent">
                ...
                <div class="col-lg-4 d-none d-lg-block text-center" data-aos="fade-left">
                    <div class="fdp-hero-image-wrapper">
                        <img src="{{ $fleetItem->image_url ?? asset('images/placeholder-fleet.jpg') }}" 
                             alt="{{ $fleetItem->title ?? 'Fleet Item' }}" 
        ...
        <div class="container">
            ...
            @elseif($errorMessage)
                <div class="fdp-error-card">
                    ...
                    <a href="{{ route('public.fleet') }}" class="btn btn-primary mt-3">
                        ...
                        <div class="fdp-content-card" data-aos="fade-up">
                            ...
                            <div class="fdp-content-body">
                                ...
                                <div class="fdp-description">
                                    {!! Str::words(strip_tags($fleetItem->description ?? ''), 80, '...') !!}
                                </div>
                                
                                @if($fleetItem->description && Str::wordCount(strip_tags($fleetItem->description)) > 80)
                                ...
                                @if($fleetItem->specifications && count($fleetItem->specifications) > 0)
                                    <h3 class="fdp-section-title mt-4">Technical Specifications</h3>
                                    <div class="fdp-specs-grid">
                                        @foreach(array_slice($fleetItem->specifications, 0, 6) as $key => $value)
                                            <div class="fdp-spec-item">
                                                <span class="fdp-spec-label">{{ $key }}</span>
                                                <span class="fdp-spec-value">{{ $value }}</span>
                                            </div>
                                        @endforeach
                                    </div>
                                    
                                    @if(count($fleetItem->specifications) > 6)
                                        <div x-show="showFullSpecs" x-transition>
                                            <div class="fdp-specs-grid mt-2">
                                                @foreach(array_slice($fleetItem->specifications, 6) as $key => $value)
                                                    <div class="fdp-spec-item">
                                                        <span class="fdp-spec-label">{{ $key }}</span>
                                                        <span class="fdp-spec-value">{{ $value }}</span>
                                                    </div>
                                ...
                                {{-- Features Section --}}
                                @if($fleetItem->features && count($fleetItem->features) > 0)
                                    <h3 class="fdp-section-title mt-4">Key Features</h3>
                                    <div class="fdp-features-list">
                                        @foreach($fleetItem->features as $feature)
                                ...
                                <div class="fdp-info-grid">
                                    @if($fleetItem->category)
                                        <div class="fdp-info-item-card">
                                            ...
                                            <span>Category</span>
                                            <strong>{{ $fleetItem->category->name }}</strong>
                                        </div>
                                    @endif
                                    @if($fleetItem->manufacturer)
                                        <div class="fdp-info-item-card">
                                            ...
                                            <strong>{{ $fleetItem->manufacturer }}</strong>
                                        </div>
                                    @endif
                                    @if($fleetItem->model_number)
                                        <div class="fdp-info-item-card">
                                            ...
                                            <strong>{{ $fleetItem->model_number }}</strong>
                                        </div>
                                    @endif
                                    <div class="fdp-info-item-card">
                                        <i class="fas fa-check-circle"></i>
                                ...
                                <div class="fdp-content-body">
                                    ...
                                    <div class="fdp-gallery-grid">
                                        @foreach($galleryImages as $index => $img)
                                            <div class="fdp-gallery-item" @click="openGallery({{ $index }})">
                                                <img src="{{ asset($img) }}" 
                                                     alt="Gallery {{ $index + 1 }}" 
                                                     loading="lazy">
                    ...
                    <div class="col-lg-4">
                        <div class="fdp-sidebar">
                            ...
                            <div class="fdp-sidebar-card fdp-sidebar-cta">
                                ...
                                <a href="{{ route('quote.index') }}" class="fdp-btn fdp-btn-accent w-100">
                            ...
                            {{-- Related Equipment --}}
                            @if($relatedFleetItems && $relatedFleetItems->count() > 0)
                                <div class="fdp-sidebar-card">
                                    <h5 class="fdp-sidebar-title">Related Equipment</h5>
                                    <div class="fdp-related-list">
                                        @foreach($relatedFleetItems as $ri)
                                            <a href="{{ route('fleet.detail', ['slug' => $ri->slug]) }}" class="fdp-related-item">
                                                <img src="{{ $ri->image_url }}" alt="{{ $ri->title }}" loading="lazy">
                                                <div>
                                                    <h6>{{ Str::limit($ri->title, 35) }}</h6>
                                                    <small>
                                                        @if($ri->category)
                                                            <i class="fas fa-layer-group me-1"></i>{{ $ri->category->name }}
                                                        @endif
                                                    </small>
                                                </div>
    ...
    <div class="fdp-gallery-modal" 
        ...
        <div class="fdp-gallery-content">
            <img src="{{ asset($galleryImages[$activeGalleryImage] ?? '') }}" 
                 alt="Gallery Image" 
